...
//              Network First para Firebase (dados sempre frescos)
// ══════════════════════════════════════════════════════════════

const CACHE_NAME = 'bertoni-pr-v1';
...
self.addEventListener('fetch', event => {
  ...
  // Firebase / Firestore → SEMPRE busca da rede (dados em tempo real)
  if (
    ...
    event.respondWith(
      fetch(event.request).catch(() => {
        // Se offline e Firebase falhar, retorna resposta de erro elegante
        return new Response(
          JSON.stringify({ error: 'offline', message: 'Sem conexão com Firebase' }),
